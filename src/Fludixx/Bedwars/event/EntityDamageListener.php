...
class EntityDamageListener implements Listener
{
    ...
    public function onDamageByEntity(EntityDamageByEntityEvent $event)
    {
        ...
        if ($player instanceof Player && $damager instanceof Player) {
            if (Bedwars::$players[$player->getName()]->getPos() > 0 && Bedwars::$players[$player->getName()]->getPos() !== Bedwars::$players[$damager->getName()]->getPos()) {
                ...
                if ($player->getHealth() <= $event->getFinalDamage()) {
                    ...
                    $player->teleport($pos);
                    Bedwars::$arenas[$levelname]->broadcast(
                        "§e" .
                        Bedwars::$players[$player->getName()]->getName() .
                        "§f was killed by §e" .
            ...
            if ($damager->getInventory()->getItemInHand()->getId() === Item::IRON_SWORD) {
                ...
                if ($mplayer->getVaule('hit') === $damager->getName()) {
                    ...
                    foreach (Bedwars::$arenas as $name => $class) {
                        if ($class->isDuelMap() && count($class->getPlayers()) === 0 && $class->getState() === Arena::STATE_OPEN) {
    ...
    public function onDamage(EntityDamageEvent $event)
    {
        ...
        if ($player instanceof Player) {
            if (Bedwars::$players[$player->getName()]->getPos() <= 0 && $event->getCause() !== EntityDamageEvent::CAUSE_VOID) {
                $event->setCancelled(TRUE);
                return;
            }
