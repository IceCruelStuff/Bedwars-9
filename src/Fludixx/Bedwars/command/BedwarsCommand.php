...
class BedwarsCommand extends Command
{
    ...
    {
        if (($sender->hasPermission("bw.admin") || $sender->isOp()) && $sender instanceof Player) {
            $player = Bedwars::$players[$sender->getName()];
            if (!isset($args[0]) || !isset($args[1]) || $args[0] == "help") {
                ...
                if ((int)$args[1][0] > 8) {
                    ...
                    if ($this->bedwars->getServer()->loadLevel($levelname)) {
                        ...
                        $player->sendMsg("Use /leave to get to the spawn");
                        Bedwars::$arenas[$player->getPlayer()->getLevel()->getFolderName()] = new Arena(
                            $player->getPlayer()->getLevel()->getFolderName(),
                            (int) $mode[2],
                            (int) $mode[0],
                            $sender->getLevel(),
                            []
